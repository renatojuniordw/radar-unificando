...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const section =
      typeof body?.section === 'string' ? body.section.slice(0, MAX_SECTION) : '';
    const jobDescription =
      typeof body?.jobDescription === 'string'
        ? body.jobDescription.slice(0, MAX_JOB_DESCRIPTION)
        : undefined;

    if (!section.trim() || section.trim().length < 3) {
      return NextResponse.json(
        { error: 'Trecho do currículo muito curto para reescrever.' },
        { status: 400 }
      );
    }
    ...
    const result = await rewriteResumeSection(resumeText, section, {
      jobDescription,
    ...
    console.error('[extension] Erro na reescrita:', error);
    const message = error instanceof Error ? error.message : 'Erro desconhecido';
    return NextResponse.json(
      { error: `Erro ao reescrever o trecho: ${message.slice(0, 300)}` },
      { status: 500 }
    );
  }
}