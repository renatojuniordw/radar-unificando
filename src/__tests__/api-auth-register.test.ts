...
describe('POST /api/auth/register', () => {
  ...
  it('should_return_409_when_email_already_exists', async () => {
    ...
    const res = await POST(makeRequest({ email: 'existing@test.com', password: '12345678' }));
  ...
  it('should_return_201_on_successful_registration', async () => {
    ...
    const res = await POST(makeRequest({ email: 'new@test.com', password: '12345678', name: 'User' }));
