...
import { provisionAdmin } from '@/lib/core/admin/admin-provisioning';
...
describe('provisionAdmin', () => {
  ...
  it('should_ensure_role_without_overwriting_password_when_admin_exists', async () => {
    const prisma = mockPrisma();
    prisma.user.findFirst.mockResolvedValue({ id: '42', email: 'admin@radar.com' });
    prisma.user.update.mockResolvedValue({ id: '42', email: 'admin@radar.com' });
  ...
  it('should_lowercase_email_before_lookup', async () => {
    ...
    await provisionAdmin(prisma, { email: 'ADMIN@Radar.COM', passwordHash: 'h', name: 'Admin' });

    expect(prisma.user.findFirst).toHaveBeenCalledWith({ where: { email: 'admin@radar.com' } });
    expect(prisma.user.create).toHaveBeenCalledWith(
  ...
  it('should_use_default_name_when_not_provided', async () => {
    ...
    await provisionAdmin(prisma, { email: 'admin@radar.com', passwordHash: 'h' });
